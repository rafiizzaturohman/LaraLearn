...
<div class="my-6 mx-auto w-auto max-w-7xl space-y-8">
    <div class="flex flex-row justify-between items-center">
        <h1 class="text-white font-bold text-2xl">Edit Barang</h1>

        <a href="{{ route('barang') }}"
            class="text-gray-200 font-bold text-md bg-gray-600 hover:text-gray-400 hover:bg-gray-800 transition ease-in-out duration-200 px-2 py-1 rounded-xs">Kembali</a>
    </div>
    ...
    <div class="bg-gray-600 p-3 rounded-sm text-white">
            <form action="{{ route('update', $barang->id) }}" method="POST">
                ...
                <div class="flex flex-col my-2 space-y-2">
                    ...
                    <input type="text"
                        class="border-b-2 border-[#7A8BA2] rounded-md px-2 py-1.5 focus:border-b-2 focus:border-white"
                        value="{{ old('kd_barang', $barang->kd_barang) }}" name="kd_barang" id="kd_barang" required>
                </div>

                <div class="flex flex-col my-2 space-y-2">
                    <label for="nama" class="font-bold tracking-wider">Nama</label>
                    <input type="text"
                        class="border-b-2 border-[#7A8BA2] rounded-md px-2 py-1 focus:border-b-2 focus:border-white"
                        value="{{ old('nama', $barang->nama) }}" name="nama" id="nama" required>
                </div>
                ...
                <div class="flex flex-col my-2 space-y-2">
                    ...
                    <input type="text"
                        class="border-b-2 border-[#7A8BA2] rounded-md px-2 py-1 focus:border-b-2 focus:border-white"
                        value="{{ old('harga', $barang->harga) }}" name="harga" id="harga" required>
                ...
                <div class="flex flex-col my-2 space-y-2">
                    ...
                    <input type="text"
                        class="border-b-2 border-[#7A8BA2] rounded-md px-2 py-1 focus:border-b-2 focus:border-white"
                        value="{{ old('stok', $barang->stok) }}" name="stok" id="stok" required>
                </div>

                <button
                    class="my-2 px-2 py-1 rounded-md bg-teal-700 hover:bg-teal-600 transition ease-in-out duration-200 font-bold tracking-wide"
